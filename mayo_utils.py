...
class mayo_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        fbp = self.transform(Image.fromarray(np.load(self.files_B[index % len(self.files_B)])))
        
        if self.aligned:
            phantom = self.transform(Image.fromarray(np.load(self.files_C[index % len(self.files_C)])))
        else:
            phantom = self.transform(Image.fromarray(np.load(self.files_C[random.randint(0, len(self.files_C) - 1)])))
